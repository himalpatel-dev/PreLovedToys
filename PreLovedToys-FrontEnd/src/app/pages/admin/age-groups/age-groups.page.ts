...
@Component({
  ...
  styleUrls: ['./age-groups.page.scss'],
  standalone: true,
...
export class AgeGroupsPage implements OnInit {
  ...
  loadData() {
    this.isLoading = true;
    this.masterService.getAllAgeGroups().subscribe({
  ...
  async openModal(item?: any) {
    ...
    const alert = await this.alertCtrl.create({
      header: isEdit ? 'Edit Age Group' : 'Add Age Group',
      inputs: [{ name: 'name', type: 'text', placeholder: 'e.g. 3-5 Years', value: item ? item.name : '' }],
      buttons: [
        { text: 'Cancel', role: 'cancel' },
        {
          ...
          handler: (data) => {
            ...
            const obs = isEdit ? this.masterService.updateAgeGroup(item.id, data) : this.masterService.createAgeGroup(data);
      ...
      buttons: [
        { text: 'Cancel', role: 'cancel' },
        {
          text: 'Delete', role: 'destructive', handler: () => {
            this.masterService.deleteAgeGroup(id).subscribe(() => { this.showToast('Deleted'); this.loadData(); });
          }
        }
      ]
    });
